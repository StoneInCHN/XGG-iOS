...
withProgressBlock:(HttpUploadProgressBlock)progressBlock
{
    ...
    AFHTTPRequestOperation * operation = [self POST:url parameters:param constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if(imageData){
            ...
            [formData appendPartWithFileData:imageData name:imageParamName fileName:name mimeType:@"image/jpeg"];
        }
  ...
  withResultBlock:(HttpPostRequestResultBlock)resultBlock
{
    ...
    [self POST:url parameters:param constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if(cardFrontPic){
            NSString *name = [NSString stringWithFormat:@"cardFrontPic%@.png", [formatter stringFromDate:date]];
            [formData appendPartWithFileData:cardFrontPic name:@"cardFrontPic" fileName:name mimeType:@"image/jpeg"];
        }
        
        if(cardBackPic){
            NSString *name = [NSString stringWithFormat:@"cardBackPic%@.png", [formatter stringFromDate:date]];
            [formData appendPartWithFileData:cardBackPic name:@"cardBackPic" fileName:name mimeType:@"image/jpeg"];
        }
  ...
  withResultBlock:(HttpPostRequestResultBlock)resultBlock
{
    ...
    [self POST:url parameters:param constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if(storePicture){
            NSString *name = [NSString stringWithFormat:@"storePicture%@.png", [formatter stringFromDate:date]];
            [formData appendPartWithFileData:storePicture name:@"storePicture" fileName:name mimeType:@"image/jpeg"];
        }
        
        if(licenseImg){
            NSString *name = [NSString stringWithFormat:@"licenseImg%@.png", [formatter stringFromDate:date]];
            [formData appendPartWithFileData:licenseImg name:@"licenseImg" fileName:name mimeType:@"image/jpeg"];
        }
        
        if (envImgs.count > 0) {
            for (NSUInteger i = 0; i < envImgs.count; i++) {
                NSData *data = [envImgs objectAtIndex:i];
                NSString *name = [NSString stringWithFormat:@"envImgs%ld%@.png",(long)i,[formatter stringFromDate:date]];
                
                if (data != NULL) {
                    [formData appendPartWithFileData:data name:@"envImgs" fileName:name mimeType:@"image/jpeg"];
                }
            }
        }
        
        
        if(commitmentImgs.count > 0){
            for (NSUInteger i = 0; i < commitmentImgs.count; i++) {
                NSData *data = [commitmentImgs objectAtIndex:i];
                NSString *name = [NSString stringWithFormat:@"commitmentImgs%ld%@.png",(long)i,[formatter stringFromDate:date]];
                
                if (data != NULL) {
                    [formData appendPartWithFileData:data name:@"commitmentImgs" fileName:name mimeType:@"image/jpeg"];
                }
            }
        }
    }  success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
          ...
          withResultBlock:(HttpPostRequestResultBlock)resultBlock
{
    ...
    [self GET:url parameters:param success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        if (responseObject) {
            id objc = [self JSONObjectWithData:responseObject];
            
            if (objc) {
                resultBlock(@"", @"", @"", objc, nil);
            }else{
                resultBlock(@"", @"", @"", nil, nil);
            }
        }
...
-(void)requestSuccessForResponseObject:(id  _Nonnull)responseObject andRequestId:(NSString*)requestId withResultBlock:(HttpPostRequestResultBlock)resultBlock
{
    if (responseObject) {
        ...
        if (responseObject) {
            ...
            if (objc) {
                code = [objc getStringValueForKey:@"code" defaultValue:@""];
                if(![[objc objectForKey:@"token"] isKindOfClass:[NSNull class]]){
                    DATAMODEL.token = [objc getStringValueForKey:@"token" defaultValue:@""];
                    if(![DATAMODEL.token isEqualToString:@""]){
                        DATAMODEL.isLogin = YES;
                    }
                }
                desc = [objc getStringValueForKey:@"desc" defaultValue:@""];
                msg = [objc valueForKey:@"msg"];
                page = [objc valueForKey:@"page"];
            }
        }
        resultBlock(requestId, code, desc, msg, page);
    }
}
...
-(void)requestFailureForError:(NSError * _Nonnull)error andRequestId:(NSString*)requestId withResultBlock:(HttpPostRequestResultBlock)resultBlock{
    ...
    if(error && error.code == -1009){
        if(resultBlock){
            resultBlock(requestId, @"-1009",@"网络未连接，请检查后重试",nil, nil);
        }
    }else{
        if(resultBlock){
            resultBlock(requestId, @"-2",@"网络传输异常，请检查后重试",nil, nil);
        }
    }
}
