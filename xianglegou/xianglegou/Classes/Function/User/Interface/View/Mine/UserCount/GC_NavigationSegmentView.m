...
@interface GC_NavigationSegmentView ()<UIScrollViewDelegate>
...
@implementation GC_NavigationSegmentView
...
-(void)initDefaultWithHeight:(CGFloat)height
{
    ...
    self.frame = CGRectMake(0, 0, kMainScreenWidth - 108, height);
...
-(void)setSelectItem:(NSInteger)item
{
    ...
    if(self.scrollView){
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.delegate customNavigationSegmentCurrentItem:item];
        });
    }else{
        [self.delegate customNavigationSegmentCurrentItem:item];
    }
...
{
    self.defalutFontColor = defaultColor;
    self.selectFontColor = selectColor;
}
...
- (UIImageView *)bottomScrollLineImage
{
    if(!_bottomScrollLineImage){
        ...
        image.frame = CGRectMake(0, self.viewHeight, WIDTH_TRANSFORMATION(120), 2.5);
        image.backgroundColor = kFontColorWhite;
