...
@interface QL_SILoadImageTableViewCell()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, Hen_PhotoCollectManagerDelegate>
...
@implementation QL_SILoadImageTableViewCell

///创建
+ (instancetype)cellWithTableView:(UITableView*)tableView
{
    static NSString *ID = @"QL_SILoadImageTableViewCell";
    QL_SILoadImageTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if(cell == nil){
        cell = [[QL_SILoadImageTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)updateUIForImageArray:(NSMutableArray *)imageArray
{
    ...
    for(NSString *imageUrl in imageArray){
        [self.inputImageArray addObject:[NSString stringWithFormat:@"%@%@", PngBaseUrl, imageUrl]];
    }
    [self updateImageInputShowIsChange:NO];
}

#pragma mark -- private

///更新图片输入
- (void)updateImageInputShowIsChange:(BOOL)isChange
{
...
-(void)successCollectPhoto:(NSMutableArray<UIImage*>*)photos
{
    ...
    [self updateImageInputShowIsChange:YES];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell updateUIForImage:self.showInputImageArray[indexPath.item] andItem:indexPath.item];
    ...
    cell.onDeleteBlock = ^(){
        ...
        [weakSelf updateImageInputShowIsChange:YES];
    };
    cell.onPngDownloadBlock = ^(UIImage *image, NSInteger item){
        if(image){
            [weakSelf.inputImageArray removeObjectAtIndex:item];
            [weakSelf.inputImageArray insertObject:image atIndex:item];
        }
    };
