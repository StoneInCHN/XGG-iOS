...
- (void)setRy_inputZeroBeginning:(NSInteger)ry_inputZeroBeginning
{
    if([self.inputView isKindOfClass:[RYNumberKeyboard class]])
        [self.inputView performSelector:@selector(setUnZeroBeg:) withObject:@(ry_inputZeroBeginning)];
    objc_setAssociatedObject(self, _cmd, @(ry_inputZeroBeginning), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)setRy_floatDecimal:(NSInteger)ry_floatDecimal
{
    if([self.inputView isKindOfClass:[RYNumberKeyboard class]])
        [self.inputView performSelector:@selector(setFloatDecimal:) withObject:@(ry_floatDecimal)];
    objc_setAssociatedObject(self, _cmd, @(ry_floatDecimal), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (NSInteger)ry_inputZeroBeginning
{
    return [objc_getAssociatedObject(self, @selector(setRy_inputZeroBeginning:)) integerValue];
}


- (NSInteger)ry_floatDecimal
{
    return [objc_getAssociatedObject(self, @selector(setRy_floatDecimal:)) integerValue];
}
