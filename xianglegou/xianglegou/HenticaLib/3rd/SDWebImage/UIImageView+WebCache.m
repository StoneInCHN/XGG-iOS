...
- (void)sd_setImageWithURL:(NSURL *)url defaultImageName:(NSString*)imageName{
    NSString *urlStr = [url absoluteString];
    if([urlStr isEqualToString:@""] || [urlStr isEqualToString:PngBaseUrl]){
        ...
        [self sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:imageName] options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            [indicatorView stopAnimating];
        }];
    }
}

- (void)sd_setImageWithUrlString:(NSString *)urlString defaultImageName:(NSString*)imageName{
    [self sd_setImageWithURL:[NSURL URLWithString:urlString] defaultImageName:imageName];
}
