...
@property (nonatomic, strong)UIPageControl *countPage;
...
- (void)showBrowseViewController
{
    ...
    //qiliu注释：iPad上退出后tableview向上移动了一段位移
...
//    [_bgView addSubview:_countLabel];
    ...
    _countPage = [[UIPageControl alloc] init];
    _countPage.frame = CGRectMake(0, _screenHeight - 50, _screenWidth, 50);
    _countPage.numberOfPages = _browseItemArray.count;
    _countPage.currentPage = _currentIndex;
    [_countPage addTarget:self action:@selector(indexPageControlChanged:) forControlEvents:UIControlEventValueChanged];
    [_bgView addSubview:_countPage];
...
///圆点点击监听
- (void)indexPageControlChanged:(id)sender{
    int page = (int)_countPage.currentPage;
    _collectionView.contentOffset = CGPointMake((_screenWidth + kBrowseSpace) * page, 0);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell)
    {
        ...
        if([browseItem.bigImage isKindOfClass:[UIImage class]]){
            // 显示大图
            [self showBigImage:cell.zoomScrollView.zoomImageView browseItem:browseItem rect:bigImageRect];
        }else if([browseItem.bigImage isKindOfClass:[NSString class]]){
            // 判断大图是否存在
            if([[SDImageCache sharedImageCache]diskImageExistsWithKey:browseItem.bigImage]){
                // 显示大图
                [self showBigImage:cell.zoomScrollView.zoomImageView browseItem:browseItem rect:bigImageRect];
            }else  // 如果大图不存在
            {
...
{
    if([browseItem.bigImage isKindOfClass:[NSString class]]){
        // 取消当前请求防止复用问题
        [imageView sd_cancelCurrentImageLoad];
        // 如果存在直接显示图片
        imageView.image = [[SDImageCache sharedImageCache]imageFromDiskCacheForKey:browseItem.bigImage];
        // 第一次打开浏览页需要加载动画
        if(_isFirstOpen){
            _isFirstOpen = NO;
...
//            }
        ...
        // 如果存在直接显示图片
        UIImage *image = browseItem.bigImage;
...
//            }else{
//                imageView.frame = CGRectMake(0, 0, ((UIImage*)browseItem.bigImage).size.width, ((UIImage*)browseItem.bigImage).size.height);
//            }
//            imageView.center = CGPointMake([UIScreen mainScreen].bounds.size.width / 2, [UIScreen mainScreen].bounds.size.height / 2);
//        }else{
//            imageView.frame = rect;
...
- (void)loadBigImageWithBrowseItem:(MSSBrowseModel *)browseItem cell:(MSSBrowseCollectionViewCell *)cell rect:(CGRect)rect
{
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:browseItem.bigImage] placeholderImage:browseItem.smallImageView.image completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
-(void)setImageViewFrame:(UIImageView*)imageView
{
    ...
    CGFloat proportion = height / width;
...
//    _countLabel = nil;
    [_countPage removeFromSuperview];
    _countPage = nil;
    
    if(self.isImageMoveClose){
...
{    // 保存图片
    ...
    else if(index == 1)
    {
        ...
        pasteboard.string = currentBwowseItem.bigImage;
